...
int array_sum[MAX_SIZE][MAX_SIZE];
int row , column;
...
for(row=0; row<MAX_SIZE; row++){
    for(column=0; column<MAX_SIZE; column++){
        array_sum[row][column]=array_1[row][column]+array_2[row][column];
}
}
printf("the sum of array 1 and 2 is\n ");
for(row=0; row<MAX_SIZE; row++){
    for(column=0; column<MAX_SIZE; column++){
   printf("%d  ",array_sum[row][column]);
}
printf ("\n");
}
